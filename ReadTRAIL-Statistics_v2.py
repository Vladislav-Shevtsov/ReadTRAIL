...
IDEALIZED MODEL (single scale, no per-locus renormalization of Q_i):
    ...
    Allele_Bases_Covered = Σ (Raw_Count_i · Fragment_Length_i)
    C_mini   = Allele_Bases_Covered / G_mini
...
ALGEBRAIC NOTE (v2.5 audit — READ THIS BEFORE INTERPRETING THE SHARE COLUMNS)
    Within one locus the two share columns are IDENTICAL, exactly:

        Idealized_Share_%  ==  Experimental_Share_%
                           ==  100 * (Raw_i / p_i) / sum_j (Raw_j / p_j)

    Verified on the 11-sample validation cohort: max |difference| over all 411
    allele rows = 0.0000000000 percentage points, and the difference from the
    plain inverse-detectability share is <= 5e-5 pp (rounding only).

    The reason is that every factor introduced between Raw_Count and the share
    is constant within a locus and therefore cancels in the ratio:
        C_mini, G_mini, N_filt, Allele_Bases_Covered  — per sample
        B_sample                                      — per sample
        B_locus_cohort                                — per locus
    So the mini-genome coverage model and the two-step cohort calibration do
    NOT contribute to the reported allele shares, and neither does the
    contamination fraction CF = 100 - Experimental_Share_%(dominant).

    They are not useless: B_sample and the cohort locus factor DO determine
    E_expected = B_sample * p* * locus_factor, which drives Locus_Completeness,
    dropout detection, the Poisson Z-score, Confidence_Score and hence the QC
    tokens.  They act on the observability layer, not on the shares.

    Both columns are kept so that existing downstream tables and figures do not
    break, but any text describing them as two different estimators of allele
...
def build_allele_level_raw(data, indices, max_alleles, sample_meta):
    ...
    for _, row in data.iterrows():
        ...
        for i in indices:
            ...
            # v2.5 diagnostic: P(L) over the INPUT read-length distribution.
            # Reported alongside p*, never substituted for it, so the two can be
            # compared on real data before any model change is adopted.
            p_pop_in    = _to_float(row.get(f'Population_Detectability_p {i}'))
            locus_eff   = _to_float(row.get(f'Locus_Efficiency {i}'))
            ...
            records.append({
                ...
                'Population_Detectability_p': p_pop_in,
                'Locus_Efficiency':          locus_eff,
                # Detectability (input)
...
def apply_idealized_model(df):
    ...
    # Allele_Bases_Covered — only the allele-sized part of every supporting
    # read: Σ(Raw_Count_i × Fragment_Length_i).  The full sequencing-read
    # length must not enter mini-genome coverage.
    # All sums include EVERY detected allele, regardless of Use_in_Idealized.
    df['_Allele_Bases_Covered_Row'] = (
        pd.to_numeric(df['Raw_Count'], errors='coerce')
        * pd.to_numeric(df['Fragment_Length_bp'], errors='coerce')
    )
    ...
    sample_agg = (df
                  ...
                  .agg(
                      ...
                      Allele_Bases_Covered=('_Allele_Bases_Covered_Row',
                                            lambda s: float(pd.to_numeric(
                                                s, errors='coerce').dropna().sum()))
                  )
                  .reset_index())

    def _c_mini(r):
        covered_bases, g = r['Allele_Bases_Covered'], r['G_mini']
        if any(_is_missing(x) for x in (covered_bases, g)) or g <= 0:
            return np.nan
        return covered_bases / g

    sample_agg['C_mini'] = sample_agg.apply(_c_mini, axis=1)
    df = df.drop(columns=['_Allele_Bases_Covered_Row'])

    df = df.merge(
        sample_agg[['Sample_ID', 'N_filt', 'G_mini',
                    'Allele_Bases_Covered', 'C_mini']],
...
ALLELE_LEVEL_COLUMNS = [
    ...
    # Mini-genome block
    'N_filt', 'G_mini', 'Allele_Bases_Covered', 'C_mini',
    # Idealized block
...
def finalize_allele_level(df):
    ...
    for c in ('N_filt', 'G_mini', 'Allele_Bases_Covered'):
        if c in df.columns:
...
def build_sample_level(df_allele, df_locus):
    ...
    for sid, grp in df_locus.groupby('Sample_ID', sort=False):
        ...
        # v2.5: a sample can legitimately have locus rows but NO allele rows —
        # e.g. Processing_Status = NO_CANDIDATE_READS, where every locus is ND.
        # Before, such a sample either never reached here (it was dropped
        # silently) or raised KeyError. It is now summarised with empty scalars.
        if sid in allele_by_sample.groups:
            sdf = allele_by_sample.get_group(sid)
        else:
            sdf = df_allele.iloc[0:0]
            logger.warning(f'Sample {sid} has no allele-level rows; all of its loci '
                           f'are reported as ND with empty sample-level scalars.')
        ...
        allele_bases = (_to_float(sdf['Allele_Bases_Covered'].dropna().iloc[0])
                        if sdf['Allele_Bases_Covered'].notna().any() else None)
        ...
        rows.append({
            ...
            'N_filt':                           n_filt,
            'G_mini':                           g_mini,
            'Allele_Bases_Covered':              allele_bases,
...
def build_interpretation_sheet(df_allele, df_locus):
    ...
    for _, lrow in df_locus.iterrows():
        ...
        rows.append({
            ...
            'Locus_ID':                      lid,
            'Locus_Name':                    lname,
            # v2.5: a locus with no detected allele reports the ND token rather
            # than an empty cell, so the column keeps one consistent type and
            # the observation is visibly present in the QC denominator.
            'Major_Allele':                  (dom_val if (dom_val and status != 'ND')
                                              else 'ND'),
...
def add_missing_locus_rows(df_locus, data):
    """
    Re-insert (Sample_ID, Locus_ID) pairs that produced no allele at all.

    v2.5 audit finding.  The locus level is derived from the allele level, so a
    locus with zero detected alleles simply had no rows and vanished from the
    publication tables — the QC matrix silently shrank instead of reporting ND.
    On the validation cohort this removed 4 of 24 x 11 = 264 observations
    (Ft-M3 in F-tul-med-18 and Ft-M8 in all three holarctica samples), while
    the manuscript reported a single ND.  A locus that yields nothing is a
    result, not an absence of one: it is reported with N_Alleles_Detected = 0,
    which _final_qc_status maps to ND.
    """
    if data is None or 'Sample_ID' not in data.columns or 'Primer' not in data.columns:
        return df_locus
    universe = {(str(s), str(p)) for s, p in zip(data['Sample_ID'], data['Primer'])}
    present = set()
    if not df_locus.empty:
        present = {(str(s), str(l)) for s, l in zip(df_locus['Sample_ID'], df_locus['Locus_ID'])}
    ...
    if 'Processing_Status' in data.columns:
        for s, st in zip(data['Sample_ID'], data['Processing_Status']):
            status_by_sample.setdefault(str(s), st)

    template = {c: None for c in (df_locus.columns if not df_locus.empty else
                                  ['Sample_ID', 'Access_Number', 'Locus_ID', 'Locus_Name',
                                   'N_Alleles_Detected', 'Total_Raw_Count'])}
    new_rows = []
    for sid, lid in missing:
        row = dict(template)
        row['Sample_ID'] = sid
        row['Locus_ID'] = lid
        if 'Locus_Name' in row:
            row['Locus_Name'] = str(lid).split('_')[0]
        row['N_Alleles_Detected'] = 0
        if 'Total_Raw_Count' in row:
            row['Total_Raw_Count'] = 0
        if 'Potential_Hidden_Allele' in row:
            row['Potential_Hidden_Allele'] = False
        if 'Denominator_Complete' in row:
            row['Denominator_Complete'] = True
        if 'Processing_Status' in row:
            row['Processing_Status'] = status_by_sample.get(sid, 'NO_CANDIDATE_READS')
        new_rows.append(row)

    logger.warning(
        f'{len(new_rows)} (sample, locus) observation(s) produced no allele and would '
        f'have been dropped from the publication tables; reporting them as ND so the '
        f'QC denominator stays complete: '
        + ', '.join(f'{s}/{l}' for s, l in missing[:10])
        + (' ...' if len(missing) > 10 else ''))
    df_new = pd.DataFrame(new_rows)
    if not df_locus.empty:
        # Align dtypes/columns explicitly so pandas does not warn about
        # concatenating all-NA columns and does not silently change dtypes.
        df_new = df_new.reindex(columns=df_locus.columns)
        for col in df_locus.columns:
            if col not in ('Sample_ID', 'Locus_ID', 'Locus_Name',
                           'N_Alleles_Detected', 'Total_Raw_Count',
                           'Potential_Hidden_Allele', 'Denominator_Complete',
                           'Processing_Status'):
                df_new[col] = df_new[col].astype(df_locus[col].dtype, errors='ignore')
        out = pd.concat([df_locus, df_new.dropna(axis=1, how='all')],
                        ignore_index=True)
    else:
        out = df_new
    return out.sort_values(['Sample_ID', 'Locus_ID']).reset_index(drop=True)
...
_DICT_ROWS = [
    ...
    ('Allele_Bases_Covered', 'allele,sample', 'Allele-sized bases covered by supporting reads', 'Σ (Raw_Count_i · Fragment_Length_bp_i)'),
    ('C_mini', 'allele,sample', 'Mini-genome coverage',                                        'C_mini = Allele_Bases_Covered / G_mini'),
...
def write_run_metadata(output_path, input_file, args, run_ts, n_samples, n_loci,
    ...
    lines = [
        ...
        '  Allele_Bases_Covered            = sum(Raw_Count_i · Fragment_Length_bp_i)',
        '  C_mini   definition            = Allele_Bases_Covered / G_mini',
...
def _apply_numeric_formats(ws):
    ...
    int_kw     = ('N_filt', 'G_mini', 'Allele_Bases_Covered',
                  'N_Alleles', 'N_Loci',
...
_ALLELE_LEVEL_DESC = [
    ...
    ('Allele_Bases_Covered',   'Allele-sized bases covered by supporting reads',          'Σ (Raw_Count_i · Fragment_Length_bp_i)', ''),
    ('C_mini',                 'Mini-genome coverage',                                    'C_mini = Allele_Bases_Covered / G_mini', ''),
...
_SAMPLE_LEVEL_DESC = [
    ...
    ('Allele_Bases_Covered',             'Allele-sized bases covered by supporting reads',        'Σ (Raw_Count_i · Fragment_Length_bp_i)', ''),
    ('C_mini',                           'Mini-genome coverage',                                  'Allele_Bases_Covered / G_mini', ''),
...
def _build_sample_meta(data):
    """
    Extract one-row-per-sample metadata.

    v2.5 input contract
    -------------------
    ReadTRAIL-Inference v2.5 reports the sequencing metrics of the INPUT
    library and those of the BBDuk candidate subset as separate columns:
    ...
    Earlier versions emitted Read_Count (input), Mean_Read_Length and
    Total_Bases_Sequenced (both actually BBDuk-candidate quantities, summed
    over every locus, i.e. 2.2-12.0 % of Read_Count in the validation cohort).
    Mean_Read_Length_bp is used here as the sample's read length for the
    baseline criteria, so taking it from the candidate subset biased it
    upwards.  The v2.5 columns are preferred; the legacy names are accepted so
    that files produced before v2.5 still load, with a warning.
    """
    sample_meta = {}
    preferred = {
        'total_bases':      ('Input_Total_Bases', 'Total_Bases_Sequenced'),
        'read_count':       ('Input_Read_Count', 'Read_Count'),
        'mean_read_length': ('Input_Mean_Read_Length', 'Mean_Read_Length'),
        'min_read_length':  ('Input_Min_Read_Length',),
        'max_read_length':  ('Input_Max_Read_Length',),
        'candidate_reads':  ('BBDuk_Candidate_Reads',),
        'candidate_bases':  ('BBDuk_Candidate_Bases',),
        'genome_coverage':  ('Genome_Coverage_Estimate',),
    }
    legacy_used = [k for k, cols in preferred.items()
                   if cols[0] not in data.columns and len(cols) > 1
                   and cols[1] in data.columns]
    if legacy_used:
        logger.warning(
            'Input appears to predate ReadTRAIL-Inference v2.5: falling back to '
            f'the legacy columns for {legacy_used}. Note that Mean_Read_Length and '
            'Total_Bases_Sequenced were computed over the BBDuk candidate subset '
            'in those files, not over the input library.')
    for _, row in data.iterrows():
        acc = str(row.get('Access_number', ''))
        if acc not in sample_meta:
            m = {}
            for key, cols in preferred.items():
                val = None
                for col in cols:
                    if col in data.columns:
                        ...
                        if val is not None:
                            break
                m[key] = val
            m['processing_status'] = row.get('Processing_Status', 'OK')
            sample_meta[acc] = m
    return sample_meta
...
def run_qc_checks(df_allele, df_locus):
    """Validate per-locus share summations and report the share equivalence."""
    issues = 0
    ...
    # v2.5: measure and report the Idealized/Experimental equivalence on THIS
    # dataset rather than asserting it from the algebra alone.
    both = df_allele[df_allele['Idealized_Share_%'].notna()
                     & df_allele['Experimental_Share_%'].notna()]
    if len(both):
        delta = (both['Idealized_Share_%'] - both['Experimental_Share_%']).abs()
        logger.info(
            f'QC: Idealized_Share_% vs Experimental_Share_% over {len(both)} allele rows: '
            f'max |difference| = {delta.max():.10f} pp, rows differing by >0.001 pp = '
            f'{int((delta > 0.001).sum())}.')
        if delta.max() <= 1e-9:
            logger.info(
                'QC: the two share columns are numerically identical on this dataset, '
                'as expected — every normalisation factor between them is constant '
                'within a locus and cancels in the ratio. Do not describe them as two '
                'independent estimators of allele proportion.')

    # v2.5: quantify the length-biased sampling of p* when the population
    # detectability is available.
    if 'Population_Detectability_p' in df_allele.columns:
        cmp_df = df_allele[df_allele['Population_Detectability_p'].notna()
                           & df_allele['p_i'].notna()
                           & (df_allele['Raw_Count'].fillna(0) >= 5)]
        if len(cmp_df) >= 10:
            excess = (cmp_df['p_i'] - cmp_df['Population_Detectability_p']) * 100
            logger.info(
                f'QC: mean p* minus population P(L) over {len(cmp_df)} alleles: '
                f'median {excess.median():+.2f} pp, 90th percentile '
                f'{excess.quantile(0.9):+.2f} pp. A positive and L-increasing excess '
                f'means the reads supporting long alleles are themselves longer '
                f'(length-biased sampling), so long alleles are under-corrected.')
...
def run_pipeline(data, input_file, args):
    ...
    #    Raw_Count=0 rows do NOT affect N_filt, G_mini,
    ...
    # v2.5: re-insert (sample, locus) observations that produced no allele so
    # they are reported as ND instead of disappearing from the QC denominator.
    df_locus = add_missing_locus_rows(df_locus, data)
